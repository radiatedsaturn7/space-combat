...
void MoveSprite(SPRITE *pSprite)
{
	...
	{	
		case NORTH:
			...
			SetSpritePosition(pSprite, pSprite->position.x, pSprite->position.y - pSprite->speed.y);
			break;
		case SOUTH:
		...
		case EAST: 
			...
			SetSpritePosition(pSprite, pSprite->position.x + pSprite->speed.x, pSprite->position.y);
			break;
		case WEST:
...
// This one is specific to your game, so feel free to change it.  
