...
unsigned char average(unsigned char *pImageBits, int x, int y)  
{  
	unsigned char ave_color;  
	unsigned char ave1, ave2, ave3, ave4, ave5, ave6, ave7;  
	ave1 = pImageBits[(y + 1) * 3 + x];  
	ave2 = pImageBits[(y - 1) * 3 + x];  
	ave3 = pImageBits[y * 3 + x + 1];  
	ave4 = pImageBits[y * 3 + x - 1];  
	ave5 = pImageBits[y * 3 + x + 2];  
	ave6 = pImageBits[y * 3 + x - 2];  
	ave7 = pImageBits[y * 3 + x];  
