	...
	public static void loadLevel (final String name) 
	{
		...
		for (int x = 0; x < lines.length; x++)
		{
			...
			if (lines[x].startsWith("createEnemy"))
			{
				...
				final GameObject spawner = PrefabFactory.createSpawner(level, spawnX, spawnY, objectToCreate);
			...
			if (lines[x].startsWith("createPlayer"))
			{
				...
				final Level l = (Level)level.getComponent(Level.class);
				l.setScrollTarget(objectToCreate);
			}
