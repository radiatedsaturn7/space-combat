...
public class Sprite
{
	public Point m_pWorldPos;
	public Point m_pTownPos;
	public Point m_pBattlePos;
	
	public int m_iOnMap;
	
	public Image m_imImage;
	...
	public Sprite()
	{
		m_imImage = Util.m_imExplosionImage;
		...
		m_pWorldPos = new Point();
	}
	
	public Sprite(Sprite s)
	{
		m_imImage = s.m_imImage;
		...
		m_iPlayerNumber = s.m_iPlayerNumber;
		m_alImage = new ArrayList(s.m_alImage);
		
		m_iOnMap = s.m_iOnMap;
		
			m_pWorldPos = new Point(s.m_pWorldPos);
		if (s.m_pTownPos != null)
			m_pTownPos = new Point(s.m_pTownPos);
		if (s.m_pBattlePos != null)
			m_pBattlePos = new Point(s.m_pBattlePos);
	}
	...
	public void Update()
	{
		if (Util.m_bOnWorldMap && m_iOnMap != 0)
		{
			m_rcBoundingBox.x = m_pWorldPos.x;
			m_rcBoundingBox.y = m_pWorldPos.y;
			m_iOnMap = 0;
		}
		if (Util.m_bOnTownMap && m_pTownPos != null && m_iOnMap != 1)
		{
			m_rcBoundingBox.x = m_pTownPos.x;
			m_rcBoundingBox.y = m_pTownPos.y;
			m_iOnMap = 1;
		}
		if (Util.m_bInBattle && m_pTownPos != null && m_iOnMap != 2)
		{
			m_rcBoundingBox.x = m_pBattlePos.x;
			m_rcBoundingBox.y = m_pBattlePos.y;
			m_iOnMap = 2;
		}
	...
	public void SetImage(int i)
	{
		...
		if (i < m_alImage.size())
			m_imImage = (Image)(m_alImage.get(i));
	}
